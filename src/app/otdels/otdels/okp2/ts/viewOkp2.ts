import { Component, OnInit,Renderer2,AfterViewInit, ElementRef,TemplateRef, ViewChild} from '@angular/core';
...
export class ModelOkp2 implements OnInit{
  ...
  async donloadAllFile(){
    ...
    var progress = 1/this.columns.Model.length*100;
     for (var row of modelServer){
      if(row.Extensions){
        var blob = await this.select.donloadFile(row.Id);
  ...
  startDonload(){
    this.serverresult = "Количество файлов в выборке" + this.columns.Model.length
    this.IsVisible = true;
